...
| `sig.level`, `insig`, `pch`, `pch.cex`, `pch.lwd`, `pch.col` | ✅ | **Off by default**, as in R. Unlike R no `p.mat` is needed: the long table carries `n` per pair, so the p-value comes from `(rho, n)`. `insig`: `pch` marks, `blank` omits the glyph, `n` does nothing. The mark is **drawn, not typed** — the cell's own two diagonals, so it is always exactly as big as what it marks; a character `x` is sized in points and so rides over its neighbours at fifty variables and floats in an empty square at ten. `pch` therefore names one mark (`cross`) rather than numbering R's symbol table, and a card asking for another is refused rather than quietly given this one. `pch.cex` scales it about the cell's centre (1.0 fills it), `pch.lwd` is its weight |
